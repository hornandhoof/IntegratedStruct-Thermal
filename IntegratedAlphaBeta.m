...
options = optimoptions('fsolve','algorithm', 'levenberg-marquardt','StepTolerance',0.001, 'Display','none','PlotFcn',@optimplotfirstorderopt)
...
for i=1:(len1/3)
    for j=1:2
        ...
        tEta0= Eta((i-1)*3+1)      ; 
        S= StrtP((i-1)*3+1+j);
        T= Transmitance((i-1)*3+1+j);
        S0= StrtP((i-1)*3+1)  ;
        T0= Transmitance((i-1)*3+1);
        fun = @(x)alphabeta(x,S,S0,T,T0,Chi,tXi,tEta,tXi0,tEta0);
        x0 = [0,0];
        [x,FVAL,EXITFLAG,OUTPUT1] = fsolve(fun,x0,options)
        hold on
        alphaM(c)= x(1);
        betaM(c)=x(2);
        tXiM(c)=tXi;
